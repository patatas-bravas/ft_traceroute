...
socket_t init_socket() {
  ...
  socket_t icmp_sock = socket(AF_INET, SOCK_DGRAM, 0);
  if (icmp_sock == -1) {
    fprintf(stderr, "[ERROR][socket]: %s\n", strerror(errno));
    return ERR_FATAL;
  }
...
int8_t send_pkt(struct sockaddr_in *dest_addr) {
  uint8_t *pkt;
...
int8_t handle_recv_pkt(const uint8_t *pkt_recv) {
...
int8_t ft_traceroute(struct sockaddr_in *addr_dest) {
  ...
  while (run) {
    if (send_pkt(addr_dest) == ERR_FATAL)
      return ERR_FATAL;

    if (recv_pkt() == ERR_FATAL)
      return ERR_FATAL;
  }
